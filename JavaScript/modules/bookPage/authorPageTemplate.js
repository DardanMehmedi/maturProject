...
function authorPage(bookInfo){
    ...
    bookTitle.innerHTML = bookInfo[0].bookTitle
        ...
        for(let i = 0; i < bookInfo[0].bookLinks.length; i++){
            let bookLinks = document.createElement(`a`);
            bookDiv.append(bookLinks)
            textDiv.append(bookLinks)
            bookLinks.classList.add(`infoStyle`)
            bookLinks.append(bookInfo[0].bookLinks[i].P)
        }
    }
